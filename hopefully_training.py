...
def doVerbTrainingTrial(agtWord, objWord, vrbWord, sentence, order, nTrial):
    ...
    if order == 'OSV':  # Correct word order for participant
        ...
        nonceText = orderSOV
        target = orderSOV
    miss_words = random.sample(nonce_words,3) # Random nonce word that does not match image, but is in the experimental language
    miss = ' '.join(miss_words)
    buttonTexts = [target, miss]
    # mix up button texts
    random.shuffle(buttonTexts)
...
def doVerbTraining():
    ...
    while i < 10:
        ...
        response, correct, buttonA, buttonB = doVerbTrainingTrial(agent, verb, object, name, order, i)
...
noncedict = {}
p = re.compile('.*\.jpg')
pathToImages = ('/Users/maryelson/Desktop/Dissertation/stimuli/images/')
